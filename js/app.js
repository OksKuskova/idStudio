(() => {
    ...
    class SelectConstructor {
        ...
        setOptionsPosition(selectItem) {
            ...
            if (!selectItem.classList.contains(this.selectClasses.classSelectOpen)) {
                ...
                selectItem.classList.remove("select_show-top");
                if (originalSelect.hasAttribute("data-scroll")) selectItemScroll.style.maxHeight = customMaxHeightValue ? `${customMaxHeightValue}px` : `200px`; else selectItemScroll.style.maxHeight = ``;
    ...
    if (iconMenu) {
        ...
        iconMenu.addEventListener("click", (function(e) {
            if (unlock) if (isMenuOpen()) app_menuClose(); else app_menuOpen();
        }));
    }
    function app_menuOpen() {
    ...
    function popupOpen(curentPopup) {
        if (curentPopup && unlock) {
            ...
            if (popupActive) popupClose(popupActive, false); else app_bodyLock();
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (window.innerWidth <= 767.98) doHide(textElement);
    }));
    window.addEventListener("resize", (() => {
        doClear(textElement);
        doInit(textElement, textElement.children[0].clientHeight);
        if (window.innerWidth <= 767.98) {
            if (textElement.hasAttribute("forcibly-opened")) return;
            doHide(textElement);
        } else doShow(textElement);
    }));
    readMoreButton.addEventListener("click", (() => {
        ...
        if (!textElement.hasAttribute(HIDDEN_ATTR)) readMoreButton.innerHTML = "HIDE"; else readMoreButton.innerHTML = "READ MORE";
    }));
    let doClear = target => {
        target.style.transition = "";
        for (let item = 0; item < target.children.length; item++) target.children[item].removeAttribute("hidden");
        target.style.height = "auto";
        viewMoreButton.style.display = "none";
        viewMoreButton.innerHTML = "VIEW MORE";
    };
    let doInit = (target, min_height) => {
        target.setAttribute(ORIGINAL_HEIGHT, target.offsetHeight);
        target.setAttribute(MIN_HEIGHT, min_height);
        target.style.overflow = "hidden";
    };
    let doHide = (target, hiddenElements = 1) => {
        target.setAttribute(HIDDEN_ATTR, "");
        viewMoreButton.style.display = "block";
        let min_height = target.getAttribute(MIN_HEIGHT);
        target.style.height = min_height + "px";
        for (let item = hiddenElements; item < target.children.length; item++) target.children[item].setAttribute("hidden", "true");
    };
    let doShow = (target, fast = false) => {
    ...
    let getCurrentTab = () => {
        for (let tab = 0; tab < bodyTabs.length; tab++) if (!bodyTabs[tab].hasAttribute("hidden")) return bodyTabs[tab];
    };
    let doReset = () => {
        ...
        if (items.length > NUM_OF_IMAGES) if (window.innerWidth <= 600) doHide(currentItems, NUM_OF_IMAGES); else doShow(currentItems);
    };
    document.addEventListener("DOMContentLoaded", (() => {
        doReset();
    }));
    tubsNavigation.addEventListener("click", (event => {
        if (event.target.closest(".tabs-portfolio__button")) setTimeout((() => {
            doReset();
        }));
    }));
    window.addEventListener("resize", (() => {
    ...
    viewMoreButton.addEventListener("click", (() => {
        let currentItems = getCurrentTab().children[0];
        if (currentItems.hasAttribute(HIDDEN_ATTR)) doShow(currentItems); else doHide(currentItems, NUM_OF_IMAGES);
        if (!currentItems.hasAttribute(HIDDEN_ATTR)) viewMoreButton.innerHTML = "HIDE"; else viewMoreButton.innerHTML = "VIEW MORE";
    }));
    window["FLS"] = true;
    isWebp();
    tabs();
